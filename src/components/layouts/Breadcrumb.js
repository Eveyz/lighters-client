...
class Breadcrumb extends Component {
  render() {
    ...
    if(action === "dashboard") {
      ...
      links = <div>
                <span style={{color: "#BDC8CB"}}><Link to="/users/admin/dashboard">Dashboard</Link> > <Link to="/courses">课程管理</Link> > 编辑课程</span>
                <h5 style={{marginTop: "10px"}}><b>编辑课程</b></h5>
              </div>
    } else if(action === "showTeacher") {
      links = <div>
                <span style={{color: "#BDC8CB"}}><Link to="/users/admin/dashboard">Dashboard</Link> > <Link to="/teachers">教师管理</Link></span>
                <h5 style={{marginTop: "10px"}}><b>查看教师</b></h5>
              </div>
    } else if(action === "showStudent") {
      links = <div>
                <span style={{color: "#BDC8CB"}}><Link to="/users/admin/dashboard">Dashboard</Link> > <Link to="/students">学生管理</Link></span>
